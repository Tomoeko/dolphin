...
FIFOAnalyzer::FIFOAnalyzer(FifoPlayer &fifo_player)
  ...
  m_object_splitter->restoreState(
      settings.value(QStringLiteral("fifoanalyzer/objectsplitter"))
          .toByteArray());
  m_search_splitter->restoreState(
      settings.value(QStringLiteral("fifoanalyzer/searchsplitter"))
          .toByteArray());
...
void FIFOAnalyzer::ConnectWidgets() {
  connect(m_tree_widget, &QTreeWidget::itemSelectionChanged, this,
          &FIFOAnalyzer::UpdateDetails);
  connect(m_detail_list, &QListWidget::currentRowChanged, this,
          &FIFOAnalyzer::UpdateDescription);

  connect(m_search_edit, &QLineEdit::returnPressed, this,
          &FIFOAnalyzer::BeginSearch);
  connect(m_search_new, &QPushButton::clicked, this,
          &FIFOAnalyzer::BeginSearch);
  connect(m_search_next, &QPushButton::clicked, this, &FIFOAnalyzer::FindNext);
  connect(m_search_previous, &QPushButton::clicked, this,
          &FIFOAnalyzer::FindPrevious);
}
...
void FIFOAnalyzer::UpdateTree() {
  ...
  if (!m_fifo_player.IsPlaying()) {
    m_tree_widget->addTopLevelItem(
        new QTreeWidgetItem({tr("No recording loaded.")}));
    return;
  }
  ...
  for (u32 frame = 0; frame < frame_count; frame++) {
    ...
    const AnalyzedFrameInfo &frame_info =
        m_fifo_player.GetAnalyzedFrameInfo(frame);
    ...
    for (u32 part_nr = 0; part_nr < frame_info.parts.size(); part_nr++) {
      ...
      if (part.m_type == FramePartType::PrimitiveData)
        ...
        object_item =
            new QTreeWidgetItem({tr("EFB copy %1").arg(part_type_nr)});
...
public:
  ...
  OPCODE_CALLBACK(void OnCP(const u8 command, const u32 value)) {
    // Note: No need to update m_cpmem as it already has the final value for
    // this object
  ...
  OPCODE_CALLBACK(void OnXF(const u16 address, const u8 count,
                            const u8 *data)) {
  ...
  OPCODE_CALLBACK(void OnIndexedLoad(const CPArray array, const u32 index,
                                     const u16 address, const u8 size)) {
    const auto [desc, written] =
        GetXFIndexedLoadInfo(array, index, address, size);
    text = QStringLiteral("LOAD INDX %1   %2")
               .arg(QString::fromStdString(fmt::to_string(array)))
               .arg(QString::fromStdString(desc));
  }
  OPCODE_CALLBACK(void OnPrimitiveCommand(
      const OpcodeDecoder::Primitive primitive, const u8 vat,
    ...
    // (another command just comes right after the current command, with no
    // vertices in between)
    const u32 object_prim_size = num_vertices * vertex_size;

    const u8 opcode = 0x80 |
                      std::to_underlying(primitive)
                          << OpcodeDecoder::GX_PRIMITIVE_SHIFT |
                      vat;
    text = QStringLiteral(
               "PRIMITIVE %1 (%2)  %3 vertices %4 bytes/vertex %5 total bytes")
               .arg(QString::fromStdString(name))
    ...
    // It's not really useful to have a massive unreadable hex string for the
    // object primitives. Put it in the description instead.
...
void FIFOAnalyzer::UpdateDetails() {
  // Clearing the detail list can update the selection, which causes
  // UpdateDescription to be called immediately.  However, the object data
  // offsets have not been recalculated yet, which can cause the wrong data to
  // be used, potentially leading to out of bounds data or other bad things.
  // Clear m_object_data_offsets first, so that UpdateDescription exits
  // immediately.
  ...
  const AnalyzedFrameInfo &frame_info =
      m_fifo_player.GetAnalyzedFrameInfo(frame_nr);
  ...
  u32 object_offset = 0;
  // NOTE: object_info.m_cpmem is the state of cpmem _after_ all of the commands
  // in this object. However, it doesn't matter that it doesn't match the start,
  // since it will match by the time primitives are reached.
  ...
  while (object_offset < object_size) {
    ...
    object_offset += OpcodeDecoder::RunCommand(
        &fifo_frame.fifoData[object_start + start_offset],
        object_size - start_offset, callback);

    QString new_label = QStringLiteral("%1:  ").arg(object_start + start_offset,
                                                    8, 16, QLatin1Char('0')) +
                        callback.text;
...
void FIFOAnalyzer::BeginSearch() {
  ...
  if (items.isEmpty() || items[0]->data(0, FRAME_ROLE).isNull() ||
    ...
    m_search_label->setText(
        tr("Invalid search parameters (no object selected)"));
    return;
  }
  ...
  if (search_str.length() % 2) {
    m_search_label->setText(
        tr("Invalid search string (only even string lengths supported)"));
    return;
  }
  ...
  for (size_t i = 0; i < length; i++) {
    ...
    if (!good) {
      m_search_label->setText(
          tr("Invalid search string (couldn't convert to number)"));
      return;
    }
  ...
  m_search_label->setText(tr("Found %1 results for \"%2\"")
...
void FIFOAnalyzer::FindNext() {
  ...
  const auto next_result =
      std::ranges::find_if(m_search_results, [index](auto &result) {
...
void FIFOAnalyzer::FindPrevious() {
  ...
  const auto prev_result = std::ranges::find_if(
      m_search_results | std::views::reverse,
...
void FIFOAnalyzer::ShowSearchResult(const size_t index) {
  ...
  QTreeWidgetItem *object_item = m_tree_widget->topLevelItem(0)
                                     ->child(result.m_frame)
                                     ->child(result.m_object_idx);
...
public:
  ...
  OPCODE_CALLBACK(void OnIndexedLoad(const CPArray array, const u32 index,
    ...
    case CPArray::XF_A:
      ...
      break;
    case CPArray::XF_B:
      // i18n: A normal matrix is a matrix used for transforming normal vectors.
      // The word "normal" does not have its usual meaning here, but rather the
      // meaning of "perpendicular to a surface".
  ...
  OPCODE_CALLBACK(void OnPrimitiveCommand(OpcodeDecoder::Primitive primitive,
                                          u8 vat, const u32 vertex_size,
                                          const u16 num_vertices,
                                          const u8 *vertex_data)) {
    const auto name = fmt::format("{} VAT {}", primitive, vat);

    // i18n: In this context, a primitive means a point, line, triangle or
    // rectangle. Do not translate the word primitive as if it was an adjective.
    ...
    u32 i = 0;
    const auto process_component =
        [&](const VertexComponentFormat cformat, ComponentFormat format,
          ...
          for (u32 j = 0; j < count; j++) {
            for (u32 component_off = 0; component_off < component_size;
                 component_off++) {
              text += QStringLiteral("%1").arg(vertex_data[i + component_off],
                                               2, 16, QLatin1Char('0'));
            }
            if (format == ComponentFormat::Float) {
              const float value =
                  std::bit_cast<float>(Common::swap32(&vertex_data[i]));
    ...
    const auto process_simple_component = [&](const u32 size) {
      for (u32 component_off = 0; component_off < size; component_off++) {
        text += QStringLiteral("%1").arg(vertex_data[i + component_off], 2, 16,
                                         QLatin1Char('0'));
      }
      i += size;
      text += QLatin1Char{' '};
    ...
    for (u32 vertex_num = 0; vertex_num < num_vertices; vertex_num++) {
      ...
      process_component(vtx_desc.low.Position, vtx_attr.g0.PosFormat,
                        vtx_attr.g0.PosElements == CoordComponentCount::XY ? 2
                                                                           : 3);
      const u32 normal_component_count =
          vtx_desc.low.Normal == VertexComponentFormat::Direct ? 3 : 1;
      const u32 normal_elements =
          vtx_attr.g0.NormalElements == NormalComponentCount::NTB ? 3 : 1;
      ...
      for (u32 c = 0; c < vtx_desc.low.Color.Size(); c++) {
        static constexpr Common::EnumMap<u32, ColorFormat::RGBA8888>
            component_sizes = {
                2, // RGB565
                3, // RGB888
                4, // RGB888x
      ...
      for (u32 t = 0; t < vtx_desc.high.TexCoord.Size(); t++) {
        process_component(
            vtx_desc.high.TexCoord[t], vtx_attr.GetTexFormat(t),
